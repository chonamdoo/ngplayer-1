...
public class VideoSurfaceView extends SurfaceView implements MediaPlayerControl {
    ...
    private FrameGrabberAsyncTask mFrameGrabberAsyncTask;
    ...
    public native int nativeGetMediaFps();
...
//            openVideo();
    ...
    private void stopVideoPlayTask() {
    	Log.v("DEBUG32","stopVideoPlayTask");
        if (mFrameGrabberAsyncTask != null) {
            mFrameGrabberAsyncTask.cancel(false);
        }
    }
    ...
    @Override
    public void start() {
        mFrameGrabberAsyncTask = new FrameGrabberAsyncTask();
        mFrameGrabberAsyncTask.execute(mPath);
    }
...
//        return mAudioSession;
    ...
    class FrameGrabberAsyncTask extends AsyncTask<String, Void, Void> {
        private String data;

        public FrameGrabberAsyncTask() {
        }
        ...
        protected Void doInBackground(String... params) {
            ...
            int result = nativePrepareBitmap(mBitmap, resolution[0], resolution[1]);
            Log.v("DEBUG32","FrameGrabberAsyncTask nativePrepareBitmap result :" + result);
            int currentFrame = 0;
            while (true) {
                long startMs = System.currentTimeMillis();
                currentFrame = nativeGetFrame();
                Log.v("DEBUG32","FrameGrabberAsyncTask nativePrepareBitmap currentFrame :" + currentFrame);
                if (currentFrame > 0) {
                    setVideoSurfaceBitmap();
                    long endMs = System.currentTimeMillis();
                    SystemClock.sleep(Math.max(0, frameTick - (int)(endMs - startMs)));
                    if (isCancelled()) {
                    	Log.v("DEBUG32","FrameGrabberAsyncTask isCancelled");
                        nativeMediaFinish(mBitmap);
                        break;
                    }
                } else {
                	Log.v("DEBUG32","FrameGrabberAsyncTask nativeMediaFinish");
                    nativeMediaFinish(mBitmap);
                    break;
                }
            }
